...
export default {
  ...
  methods:{
    ...
    getMyTasKCreate(){
      let data = []
      this.axios.get(this.api.baseUrl + "/task/getMyTaskCreate/"+ this.$cookies.get("userID") +"/"+this.$cookies.get("userCompany")).then((res) => {
        ...
        if (code === 200) {
          for (let i = 0; i < res.data.data.length;i++){
            ...
            for (let j=0;j<this.workUsers.length;j++){
              ...
              if (this.workUsers[j].value===parseInt(data[i].taskCreator)){
                data[i].taskCreator = this.workUsers[j].label;
              }
            }
  ...
  created() {
    ...
    this.data2 = this.getMyTasKCreate();
    this.data1 = this.getMyTasKCreate();
  }
}
</script>
