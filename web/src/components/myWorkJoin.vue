...
export default {
  ...
  methods:{
    ...
    getMyTaskJoin(){
      let data = []
      this.axios.get(this.api.baseUrl + "/task/getMyTaskJoin/"+ this.$cookies.get("userID") +"/"+this.$cookies.get("userCompany")).then((res) => {
  ...
  created() {
    ...
    this.data2 = this.getMyTaskJoin();
    this.data1 = this.getMyTaskJoin();
  }
}
</script>
